<?php
	...
	define("SITE", "http://localhost/ytelefon");// <-- site adresi. Sonunda "/" olmamalı!
	define("LIMIT", "10");						// <-- her sayfada görüntülenecek kayıt sayısı
	define("EZSQL_DB_USER", "root");			// <-- veritabanı kullanıcı adı
	define("EZSQL_DB_PASSWORD", "");			// <-- veritabanı kullanıcı parolası
	define("EZSQL_DB_NAME", "ytelefon");		// <-- veritabanı ismi
	define("TABLO", "bilgiler");				// <-- veritabanı tablosu
	define("EZSQL_DB_HOST", "localhost");		// <-- veritabanı sunucusu
  ...
  if ($sayfasayisi > 1) {   
	...
	class db {
		...
		function db($dbuser, $dbpassword, $dbname, $dbhost)
		{
			...
			if ( ! $this->dbh )
			{
				$this->print_error("<b>Veritabanı bağlanamıyor!</b>");
			}
		...
		function select($db)
		{
			...
			{
				$this->print_error("<u>$db</u>! veritabanı seçilemiyor!");
			}
		}
